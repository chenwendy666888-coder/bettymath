...
2. ✅ **② 完整视觉扫描光剑卷**（已完成）：扫描 5 套光剑数学卷 56 题 → 产出 `syllabus/guangjian-exam-analysis.html`(+md，含数据仪表盘)。高频：相似×10·坐标几何×9·一元二次×9·四边形×8；英文卷加料占 50–64%。已 surface 进 map.html(节点带 ×N 考频) 与主考纲。（剔除了 4 份实为英语学科的"光剑英语测试卷"。）
3. **③ 知识点 ↔ 真题 双向对应（← 下一个）**：基于②的 56 题全题清单，每个知识点挂上"哪套卷哪题考它"，反向每题挂知识点；surface 进知识图谱/hub（点知识点→看真题；点真题→看考点）。
